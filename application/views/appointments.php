				...
				<?php foreach ($citas->result() as $cita) { ?>
					...
					<div class="row">
						...
						<div class="col-lg-2">
							...
							<i class="fas fa-notes-medical"></i><label for="notas">&nbsp;Notas: <?php echo $cita->notas; ?></label>
				...
				<?php foreach ($citas->result() as $cita) { ?>
					...
					<?php if($cita->idUsuario == $this->session->userdata('id')){?>
						...
						<div class="row">
							...
							<div class="col-sm-4 container text-center">
								<button style="font-weight: bold;  background-color: #5cb85c;" type="button" class="btn btn-info navbar-brand" data-toggle="modal" data-target="#ModalER<?php echo $cita->idCita;?>">Escribir reseña</button>
							</div>
									...
									<div class="modal-body border-rounded my-1 p-4">
								        ...
								        <form action="<?=base_url();?>index.php/Principal/modificarCita/<?=$cita->idCita?>" method="post">
								        	<div class="md-form">
									            ...
									            <input id="fechaN" name="fechaN" class="form-control" type="date" required="" value="<?php echo $cita->fecha; ?>">
						...
						<div class="modal fade" id="ModalER<?php echo $cita->idCita;?>" tabindex="-1" role="dialog" aria-hidden="true">
							<div class="modal-dialog modal-dialog-centered" role="document">
								<div class="modal-content">
									...
									<div class="modal-body border-rounded my-1 p-4">
										...
										<!--Título-->
								        <h4 class="text-center font-weight-bold">Por favor deje sus comentarios</h4>

								        <br>

								        <!--Imágen-->
								        <div class="text-center">
								          <img src="<?php echo base_url(); ?>img/reseñas.png" class="img-fluid mx-auto d-block rounded-circle img-responsive" style="width:40%">
								        </div>
										...
										<?php if ($dentistas != FALSE) {
											...
											foreach ($dentistas->result() as $dentista) {?>
												...
												<?php if($cita->idDentista == $dentista->idDentista){?>
													...
													<!--Formulario para solicitar datos-->
													<form action="<?=base_url();?>index.php/Principal/crearR/<?=$dentista->idDentista?>" method="post">
														<div class="md-form">

														<h2 id="nombre" name="nombre">Dr(a). <?php echo $dentista->nombre;?>&nbsp;<?php echo $dentista->apellidos; ?></h2>

														<i class="fas fa-edit"></i><label for="reseña">&nbsp;Reseña</label>
														<input id="reseña" name="reseña" class="form-control" type="text" required="" placeholder="Por favor proporcione un comentario...">

														<br>

														<i class="fas fa-star"></i><label for="calificacion">&nbsp;Calificaci&oacute;n</label>
												        ...
												        <div class="container" style="margin-top:1em">
												        	<div class="row">
												        		...
												        		<div class="col">
												        			<div class="text-center">
												        				<button type="submit" class="btn btn-secondary btn-lg active"><i class="fas fa-pencil-alt"></i> Escribir reseña</button>
												        			</div>
												        		</div>
												          	</div>
												          </div>
